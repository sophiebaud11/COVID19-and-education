...
pie_chart <- ggplot(pie_data, aes(x = "", y = values, fill = answer)) +
  geom_bar(stat="identity", width=1, color="white") +
  coord_polar("y", start=0) + theme_void() +
  scale_fill_brewer(palette="Accent", name = "Preference") 
...
bar_chart_remote <- bar_chart_remote_data %>%
  ggplot(aes(x = Question, y = Agree, fill = Agree)) + geom_col() + 
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) + 
  theme_minimal() + theme(legend.position = "none") + 
  labs(x = "", title = "Concerns about Remote Teaching", 
       caption = "Source: Ipsos/NPR") + scale_fill_viridis_c(direction = -1,
                                                             begin = 0.5,
                                                             end = 0.9)
...
bar_chart_inperson <- bar_chart_inperson_data %>%
  ggplot(aes(x = fct_reorder(as.factor(Question), desc(Agree)), 
...
saveRDS(pie_data, "pie_data.RDS")

districtComms <- ggplot(communication_chart_data, 
  ...
  theme(legend.position = "none") + scale_fill_viridis_d(end = 0.9)
...
final_fit_data <- left_join(final_sample_data, regions, 
                            by = "state") %>%
...
final_fit_table <- final_fit_data %>%
  select(school.district, covid.restriction, `COVID Rate`, 
         Region, county, state) %>%
  group_by(county) %>%
  summarize(`Mean COVID Restriction Level`= mean(covid.restriction, na.rm = TRUE), 
...
final_sample_data_binomial <-readRDS("final_sample_data.RDS")
...
exp(-3.81122 + (-0.22950 * (0.01))) / (1 + exp(-3.81122 + (-0.22950 * (0.01))))
 
exp(-2.86936 + (-0.22950 * (0.01))) / (1 + exp(-2.86936 + (-0.22950 * (0.01))))
